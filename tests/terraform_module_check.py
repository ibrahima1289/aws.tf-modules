"""Simple Terraform modules hygiene check: file types + terraform fmt + module completeness."""
import argparse
import os
import shutil
import subprocess
...
# Files every complete module must have
REQUIRED_MODULE_FILES = {"main.tf", "variables.tf", "outputs.tf", "providers.tf"}
...
def check_module_completeness(modules_root: Path) -> List[str]:
    """Return a list of issues for modules with empty main.tf or missing required files."""
    issues: List[str] = []
    for root, dirs, files in os.walk(modules_root):
        dirs[:] = [d for d in dirs if d not in IGNORE_DIR_NAMES]
        ...
        # Check for empty main.tf (stub / unimplemented module)
        main_tf = module_path / "main.tf"
        if main_tf.stat().st_size == 0:
            issues.append(f"EMPTY main.tf in {module_path}")
        # Check for missing required files
        for req in REQUIRED_MODULE_FILES:
            if req not in files:
                issues.append(f"MISSING {req} in {module_path}")
    return issues
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Check Terraform module folders for allowed files, formatting, and completeness")
    ...
    # Check module completeness (empty main.tf / missing required files)
    print("\nModule completeness check:")
    completeness_issues = check_module_completeness(modules_root)
    if completeness_issues:
        print("Module completeness issues found:")
        for issue in completeness_issues:
            print(f" - {issue}")
    else:
        print("All modules with main.tf are complete.")
    ...
    if disallowed:
        ...
        return 3
    if completeness_issues:
        print("\nResult: FAIL (module completeness issues found)")
        return 5
    if not ok_fmt:
