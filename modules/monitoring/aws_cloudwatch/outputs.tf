...
output "log_group_names" {
  ...
  value       = { for k, lg in aws_cloudwatch_log_group.log_group : k => lg.name }
}

output "log_group_arns" {
  description = "Map of log group key to ARN"
  value       = { for k, lg in aws_cloudwatch_log_group.log_group : k => lg.arn }
}

# ─── Metric Alarms ───────────────────────────────────────────────────────────
output "metric_alarm_arns" {
  description = "Map of metric alarm name to ARN"
  value       = { for k, a in aws_cloudwatch_metric_alarm.metric_alarm : k => a.arn }
}

output "metric_alarm_ids" {
  description = "Map of metric alarm name to resource ID"
  value       = { for k, a in aws_cloudwatch_metric_alarm.metric_alarm : k => a.id }
}

# ─── Composite Alarms ────────────────────────────────────────────────────────
output "composite_alarm_arns" {
  description = "Map of composite alarm name to ARN"
  value       = { for k, a in aws_cloudwatch_composite_alarm.composite_alarm : k => a.arn }
}

output "composite_alarm_ids" {
  description = "Map of composite alarm name to resource ID"
  value       = { for k, a in aws_cloudwatch_composite_alarm.composite_alarm : k => a.id }
}
...
output "log_metric_filter_ids" {
  ...
  value       = { for k, f in aws_cloudwatch_log_metric_filter.log_metric_filter : k => f.id }
}

# ─── Log Subscription Filters ────────────────────────────────────────────────
output "log_subscription_filter_ids" {
  description = "Map of log subscription filter name to resource ID"
  value       = { for k, f in aws_cloudwatch_log_subscription_filter.log_subscription_filter : k => f.id }
}
