...
- Refactor: [CloudWatch module](modules/monitoring/aws_cloudwatch/README.md) resource labels renamed from `"this"` to descriptive names (`log_group`, `metric_alarm`, `composite_alarm`, `dashboard`, `log_metric_filter`, `log_subscription_filter`) in `main.tf` and `outputs.tf` for improved readability; existing applied stacks require `terraform state mv` per resource to avoid destroy/recreate.
- Enhancement: CloudWatch wrapper dashboard body externalized to [`tf-plans/aws_cloudwatch/dashboards/demo-overview.json`](tf-plans/aws_cloudwatch/dashboards/demo-overview.json); loaded via `file()` in `locals.tf`; `concat(local.file_dashboards, var.dashboards)` merges file-sourced and variable-sourced dashboards — no inline JSON required in `terraform.tfvars`.
- Enhancement: `demo-overview.json` expanded to 6 widget types across 4 rows: `text` (Markdown header with runbook link), `metric` timeSeries (EC2 CPU with alarm threshold annotation), `metric` stacked area (Lambda Invocations / Errors / Throttles), `metric` singleValue with sparkline (Lambda P99 duration + error count), `alarm` status badges, and `log` Logs Insights query.
